...
---
**关键词**：$KV\ Cahce$，内部/外部碎片，动态批处理，逻辑/物理块，块表[^1]，$BlockManager$，$Scheduler$，内存共享。
...
在 $vLLM$ 出现之前，现有系统普遍采用静态批处理（$Static\ Batching$）。系统为了处理一个 $batch$，会预先分配一块连续的、巨大的内存空间给这个 $batch$ 中的每一个请求。并且，这个空间的大小是按照可能的最长回复来计算的，然而大部分请求根本不会生成如此长的回复，导致预留的内存空间大量被闲置浪费（内存内部碎片化[^2]）。
![alt text](pageattention.assets/1.png)

另外，原文[^3]中提到，现有 LLM 服务通常采取并行采样和波束搜索[^4]，为每个请求生成多个输出，此时部分序列本可以共享 KV 缓存，但是受到前文提到的 KV 缓存单独连续存储的限制，现有系统无法实现该功能。
...
随着更多 $token$ 及其 KV 缓存的生成，vLLM 会动态地将新的物理块分配[^6]给逻辑块。由于所有块都是从左到右填充的，并且只有在之前的所有块都已满时才会分配一个新的物理块，因此 vLLM 将请求的所有内存浪费限制在一个块内，请求生成完毕后，可以释放其 KV 块来存储其他请求的 KV 缓存。

并且请求可以进行动态批处理[^7]，从而提高吞吐量。
...
[^1]: 块表即映射表，每个序列都有一张块表，`block_table[i]`存储的就是第 i 个逻辑块对应的物理块在显存中的地址。

[^2]: 当请求申请了一片巨大的连续内存空间后，剩余空间可能不足以再用于其他进程的申请，此称为“外部碎片”；并且，在申请的空间内部，请求也无法完全利用所有的内存，此称为“内部碎片”。

[^3]: [《Efficient Memory Management for Large Language Model Serving with PagedAttention》](https://ar5iv.labs.arxiv.org/html/2309.06180)

[^4]: [贪心搜索、集束搜索、随机采样](https://blog.csdn.net/jiangchao98/article/details/124934656)

[^5]: $BlockManager$ 维护着一个统一的物理块池，负责整个系统的物理块的分配和释放。当 $Scheduler$ 为一个新的序列分配资源时，会向 $BlockManager$ 申请所需的物理块；当一个序列生成结束被释放时，其占用的物理块也会被 $BlockManager$ 回收。

[^6]: $Scheduler$（调度器）负责接收所有用户的请求，并决定在下一个推理步骤中，执行哪些请求。它的决策主要依据是当前 $BlockManager$ 中剩余的空闲物理块数量：如果资源充足，它可以将等待队列中的新请求加入到运行队列中；如果资源紧张，它可能会暂停某些请求。

[^7]: 动态批处理指服务器可以根据当前请求量，动态地将多个请求组合成一个批次进行处理。
      在 vLLM 中还实现了连续批处理，即在一个批次的处理完成之后，系统无需等待整个批次的所有序列都生成完毕，可以立即将已完成的序列从批次中移除，并用等待队列中的新序列填补空位，然后开始下一个迭代。