...
> 1. 操作系统的定义？
  操作系统的边界是相对的，除去**应用、硬件**之外的都可以称作操作系统。例如 Java 虚拟机跑在安卓上就属于操作系统的一部分，而跑在 Linux 上就属于应用。
> 2. 操作系统的功能？
>   - **服务**应用、**管理**应用
>   - **管理**硬件、**抽象**硬件
...
<div align="center">
  <img src="sjtu.assets/buddy-system.png"
  width="100%">
</div>

如果页面大小为 4 KiB，则不同阶对应的**内存块**大小如下：
...
free_area[2]：所有 4 页的空闲块
free_area[3]：所有 8 页的空闲块
……
...
比如，申请 1 页，但 order 0 为空，而 order 2 中存在一个 4 页空闲块：
...
$$
2^{12}\times2^{order}=2^{12+order}\text{ B}
$$

设**当前块物理地址** addr，则 Buddy 的物理地址：
